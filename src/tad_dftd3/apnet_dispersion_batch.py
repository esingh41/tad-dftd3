...
def cn_d3_batch(
    ...
    **kwargs: Any,
    ...
    #Wait, I don't want to sum right now because then it builds the matrix
    print(f"{cf_B = }")
...
def apnet_dispersion_batch(
    ...
    **kwargs,
    ...
    #Okay, so I built the coordination numbers 
    #from only intramonomer information, and now I am 
    #trying to get the weights. So the weights are going to inform
    #what C6 I get for each pair.
    #So the weights
    # 
    # 
    # 
    ...
    #Now I have two coordination numbers associated with each 
    #intermolecular edge. Now I want C6 for each intermolecular edge right?
    cn_A = cn_A.index_select(0, batch.e_ABsr_source)
    cn_B = cn_B.index_select(0, batch.e_ABsr_target)

    print(f"{cn_A = }")
    print(f"{cn_B = }")
    return
    ZA = ZA.index_select(0, batch.e_ABsr_source)
    ZB = ZB.index_select(0, batch.e_ABsr_source)
    ...
    #ASK MENTOR, only using short range edges right now
    e_ABsr_source = batch.e_ABsr_source
    e_ABsr_target = batch.e_ABsr_target
    # e_AB_full_source = batch.e_ABfull_source
    # e_AB_full_target = batch.e_ABfull_target
    distances = get_distances(RA=RA, RB=RB, e_source=e_ABsr_source, e_target=e_ABsr_target)
    print(distances[0])

    #C8 is computed recursively from c6
    ZA = ZA.index_select(0, e_ABsr_source)
    ZB = ZB.index_select(0, e_ABsr_target)
